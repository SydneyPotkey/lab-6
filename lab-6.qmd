...
remove_outliers <- function(data, ..., sd_thresh = 3) {
  ...
  for (var in vars) {
    col <- eval_tidy(var, data)
    z <- (col - mean(col, na.rm = TRUE)) / sd(col, na.rm = TRUE)
...
variables in a data set. The user should be able to supply the data set, the 
...
impute_missing <- function (data, ..., .f = mean ){
  vars <- enquos(...)
}
